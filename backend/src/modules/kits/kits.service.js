...
class KitsService {
  ...
  /**
   * Deliberately NOT a single `prisma.kit.create({ data: { components: {
   * create: [...] } } })` — the tenant-scoping extension (see
   * config/db.js) only injects business_id onto the top-level model of
   * a query; a nested relation write like `components: { create: [...] }
   * }` creates KitComponent rows the extension never sees as their own
   * operation, so they'd be written with no business_id at all (that
   * column is NOT NULL — this is exactly what broke kit creation after
   * multi-tenancy landed). Instead: create the Kit, then create its
   * components with their own top-level `kitComponent.createMany()`
   * call — same pattern update() below already used, which is why
   * editing a kit's components never hit this bug. Wrapped in a
   * transaction purely for atomicity (so a failure between the two
   * writes can't leave a kit with zero components behind).
   */
  async create({ name, sku, kitPrice, components }) {
    ...
    const kit = await prisma.$transaction(async (tx) => {
      const createdKit = await tx.kit.create({
        data: { name, sku, kit_price: Number(kitPrice) },
      });
