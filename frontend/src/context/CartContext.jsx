...
export function CartProvider({ children }) {
  ...
  // What checkout will actually require as the minimum paid amount — this
  // is the number shown to the cashier as "Total", and it must match
  // sales.service.js#checkout exactly or the two disagree on what's
  // owed. Rounded to the nearest whole rupee (standard cash-rounding —
  // nobody hands over paisa in change) rather than left at 2 decimal
  // places, which is what previously produced totals like 6733.21 and
  // then auto-filled "Paid Amount" with that same odd number.
  const total = useMemo(() => Math.round(subtotal - discountTotal + taxTotal), [subtotal, discountTotal, taxTotal])
