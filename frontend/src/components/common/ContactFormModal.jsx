...
export default function ContactFormModal({ isOpen, onClose, onSave, initialValues, title, entityLabel }) {
  ...
  const [form, setForm] = useState({ name: '', phone: '', address: '', customerType: 'RETAIL' })
  ...
  function handleSubmit(e) {
    ...
    if (isCustomer) payload.customer_type = form.customerType
    onSave(payload)
  }
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={title} size="sm">
      <form onSubmit={handleSubmit} className="space-y-4" noValidate>
        ...
        {isCustomer && (
          <div>
            <label className="label-text" htmlFor="contact-type">
              Customer Type
            </label>
            <select
              id="contact-type"
        ...
        <div>
          ...
          />
          {errors.phone && <p className="text-xs text-rose dark:text-dark-rose mt-1">{errors.phone}</p>}
        </div>
        <div>
