...
export default function StatCard({ label, value, icon, tone = 'ink', suffix, highlight = false }) {
  const iconToneClass = {
    ink: 'bg-gradient-to-br from-steel to-steel-dark dark:from-dark-steel dark:to-steel-dark text-white',
  ...
  const accentBarClass = {
    ink: 'bg-steel dark:bg-dark-steel',
    amber: 'bg-amber',
    teal: 'bg-teal dark:bg-dark-teal',
    rose: 'bg-rose dark:bg-dark-rose',
  }[tone]
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 relative z-[2]">
        <p className="text-[11px] sm:text-xs font-medium text-ink-muted dark:text-dark-muted uppercase tracking-wide truncate">
          {label}
        </p>
        <p className="figure text-xl sm:text-2xl font-semibold text-ink dark:text-dark-text mt-2 transition-transform duration-300 group-hover:translate-x-0.5">
          {value}
          {suffix && <span className="text-sm font-normal text-ink-muted dark:text-dark-muted ml-1">{suffix}</span>}
        </p>
      </div>
      <div
