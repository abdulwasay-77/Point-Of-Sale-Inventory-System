  ...
  return (
    <ul className="divide-y divide-line dark:divide-dark-border">
      {products.map((product) => (
        <li
          key={product.id}
          className="group flex items-center justify-between gap-3 py-3 px-3 -mx-2 rounded-lg transition-all duration-200 hover:bg-gradient-to-r hover:from-rose-light/70 hover:to-transparent hover:shadow-[inset_3px_0_0_0_#C1502E] dark:hover:from-dark-rose/15 dark:hover:shadow-[inset_3px_0_0_0_#E2795A]"
        >
          <div className="flex items-center gap-3 min-w-0">
            <span className="hidden sm:flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-rose-light dark:bg-dark-rose/15 text-rose dark:text-dark-rose ring-1 ring-rose/20 dark:ring-dark-rose/20 transition-transform duration-300 group-hover:scale-110 group-hover:rotate-3">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-ink-muted dark:text-dark-muted figure">{product.sku}</p>
            </div>
          </div>
