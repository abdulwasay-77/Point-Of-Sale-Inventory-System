    ...
    <div className="relative" ref={wrapperRef}>
      <button
        ...
        className={`group flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-sm font-medium transition-colors ${
          isOpen ? 'bg-amber-light dark:bg-amber/15 text-amber-dark dark:text-amber' : 'text-ink-muted dark:text-dark-muted hover:bg-paper-dim dark:hover:bg-dark-card2 hover:text-ink dark:hover:text-dark-text'
        }`}
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-[340px] max-w-[calc(100vw-2rem)] h-[440px] card chat-panel-glow flex flex-col overflow-hidden z-30">
          <div className="px-4 py-3 border-b border-line dark:border-dark-border bg-gradient-to-r from-ink to-ink-light text-paper flex items-center gap-2.5 shrink-0">
            ...
            <div className="min-w-0">
              ...
              <span className="flex items-center gap-1.5 text-[11px] text-paper/60 leading-tight mt-0.5">
                <span className="h-1.5 w-1.5 rounded-full bg-teal dark:bg-dark-teal pulse-dot-teal" />
                Ready to help
              </span>
            </div>
          </div>
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto scrollbar-hide p-3 space-y-2.5">
            {messages.map((m, i) => (
              <div key={i} className={`flex items-end gap-2 ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                {m.role === 'bot' && (
                  <span className="h-6 w-6 rounded-full bg-amber-light dark:bg-amber/15 text-amber-dark dark:text-amber flex items-center justify-center shrink-0">
                    <Icon name="chat" className="h-3.5 w-3.5" />
                  </span>
                )}
                <div
                  className={`max-w-[78%] rounded-lg px-3 py-2 text-sm whitespace-pre-line ${
                    m.role === 'user' ? 'bg-amber text-ink dark:text-dark-text' : 'bg-paper-dim dark:bg-dark-card2 text-ink dark:text-dark-text'
                  }`}
                >
                  {m.text}
                </div>
            ...
            {isSending && (
              <div className="flex items-end gap-2 justify-start">
                ...
                <div className="bg-paper-dim dark:bg-dark-card2 rounded-lg px-3 py-2.5 flex items-center gap-1" aria-label="Assistant is typing">
          ...
          <form onSubmit={handleSend} className="p-3 border-t border-line dark:border-dark-border flex gap-2 shrink-0">
            <input
