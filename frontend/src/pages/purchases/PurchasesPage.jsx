...
export default function PurchasesPage() {
  ...
  const canRecord = has('PURCHASES_CREATE')
  ...
  // Called by the New Purchase modal after it creates a brand-new product
  // inline (see NewPurchaseModal's "New Product" button below). Appends it
  // to this page's product list so it's immediately selectable in the
  // purchase-line dropdown, without waiting on a full loadData() round trip.
  ...
  return (
    <div>
      ...
      {suggestedPrices.length > 0 && (
        <div className="rounded-xl border border-amber/40 bg-amber-light/40 dark:bg-amber/15 px-4 py-3 mb-5 sm:mb-6">
          <p className="text-sm font-semibold text-ink dark:text-dark-text mb-2">
          ...
          <ul className="space-y-2">
            {suggestedPrices.map((s) => (
              <li key={s.productId} className="flex flex-wrap items-center justify-between gap-2 text-sm bg-white/60 dark:bg-dark-card/60 rounded-lg px-3 py-2">
                <div className="min-w-0">
                  <span className="font-medium text-ink dark:text-dark-text">{s.productName}</span>
                  <span className="text-ink-muted dark:text-dark-muted ml-2 figure">
      ...
      <div className="card card-premium glow-amber">
        {isLoading ? (
          ...
          <EmptyState
            ...
            actionLabel={canRecord ? 'New Purchase' : undefined}
            onAction={canRecord ? formModal.open : undefined}
            icon="🧾"
          />
        ) : (
              ...
              <tbody>
                {paginated.map((purchase) => (
                  <tr key={purchase.id} className="group">
                    <td className="text-ink-muted dark:text-dark-muted">{formatDate(purchase.date)}</td>
                    <td>
 ...
 *  "New Product" button below opens the exact same ProductFormModal used
 *  there. Once saved, the new product is added to the dropdown (via
 *  onProductCreated, which updates the parent page's product list) and a
 *  fresh line is added with it already selected, ready for qty/cost. */
function NewPurchaseModal({ isOpen, onClose, onSave, suppliers, products, warehouses, onProductCreated }) {
  ...
  useEffect(() => {
    ...
    // Product" button below updates the parent's `products` list while
    // this modal is still open, and re-running this reset on that change
    // would wipe out the line it just added.
  ...
  return (
    <>
      <Modal
        isOpen={isOpen}
        // While the nested "New Product" form is open on top of this one,
        // ignore Escape here so it only closes that top modal — otherwise
        // both would close at once and the in-progress purchase would be lost.
        onClose={newProductModal.isOpen ? () => {} : onClose}
        title="New Purchase"
        size="lg"
      >
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div>
          ...
          {productFormError && (
            <p className="text-xs text-rose dark:text-dark-rose bg-rose-light dark:bg-dark-rose/15 rounded-lg px-2.5 py-1.5 mb-2">{productFormError}</p>
          )}
          ...
          <div className="space-y-3">
            {lines.map((line, index) => {
              ...
              return (
                <div
                  key={index}
                  className="rounded-lg border border-line dark:border-dark-border p-2.5 space-y-2 transition-all duration-200 hover:border-amber/50 hover:shadow-[0_4px_14px_-4px_rgba(232,163,61,0.25)]"
                >
                    ...
                    >
                      {products.map((p) => (
                        <option key={p.id} value={p.id}>
                          {p.name}
                          {p.isVariantTracked ? ` (${p.variationName})` : ''}
        ...
        <div className="receipt-panel card-premium glow-amber flex items-center justify-between px-4 py-3.5">
          <span className="text-sm text-ink-muted dark:text-dark-muted">Total cost</span>
