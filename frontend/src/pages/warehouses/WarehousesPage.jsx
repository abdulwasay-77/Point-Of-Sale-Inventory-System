        ...
        {TABS.map((t) => (
          <button
            ...
            className={`flex items-center gap-1.5 px-4 py-2.5 text-sm font-medium border-b-2 -mb-px transition-all duration-200 ${
              tab === t.id ? 'border-amber text-ink dark:text-dark-text' : 'border-transparent text-ink-muted dark:text-dark-muted hover:text-ink dark:hover:text-dark-text'
            }`}
          >
...
function WarehousesTab() {
  ...
  const canManage = has('WAREHOUSES_MANAGE')
                  ...
                  <tr key={w.id} className="group">
                    <td>
                      <div className="flex items-center gap-3">
                        <span className="section-icon rounded-lg bg-ink text-paper dark:bg-dark-border dark:text-dark-text">
                          <Icon name="suppliers" className="h-4 w-4" />
                        </span>
                        <span className="font-medium">{w.name}</span>
                      </div>
                    </td>
                    <td className="text-ink-muted dark:text-dark-muted">{w.address || '—'}</td>
...
function TransfersTab() {
  const { has } = usePermissions()
  const canCreate = has('TRANSFERS_CREATE')
  ...
  return (
    <div>
      ...
      {canCreate && warehouses.length < 2 && (
        <p className="text-sm text-ink-muted dark:text-dark-muted mb-4">Add a second warehouse location before creating transfers.</p>
      )}
              ...
              <tbody>
                {transfers.map((t) => (
                  <tr key={t.id} className="group">
                    <td className="text-ink-muted dark:text-dark-muted">{formatDateTime(t.date)}</td>
                    <td className="font-medium">
                      {t.product}
                      {t.batch && <span className="text-ink-muted dark:text-dark-muted text-xs ml-1.5">· {t.batch}</span>}
                    </td>
                    <td>{t.from}</td>
                    <td>
                      <span className="inline-flex items-center gap-1.5">
                        <Icon name="chevronDown" className="h-3.5 w-3.5 -rotate-90 text-teal-dark dark:text-dark-teal" />
                        {t.to}
                      </span>
                    </td>
      ...
      <Modal isOpen={formModal.isOpen} onClose={formModal.close} title="New Stock Transfer" size="sm">
        <form onSubmit={handleCreate} className="space-y-4">
          <div className="grid grid-cols-[1fr_auto_1fr] gap-2 items-end">
            ...
            <span className="h-9 w-9 flex items-center justify-center rounded-full bg-teal-light dark:bg-dark-teal/15 text-teal-dark dark:text-dark-teal shrink-0">
